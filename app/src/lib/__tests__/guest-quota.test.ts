...
describe("聞けなかったとき", () => {
  // 🔴 **2026-09-03 に期待値を反転した。** ここは 2026-08-31 まで
  //    「通すが書き戻さない」を**正解として固定していた**。
  //
  //    その前提は「全面拒否にしない（設定ミスで全員止まる）／
  //    無制限にはならない —— 利用者側（DB）の 3 件が必ず残る」だった。
  //    **後半が誤りだった** —— 匿名 user_id は公開 API で作り直せるので、
  //    DB 側の関門も一緒にリセットされる。**1 件ずつ無限に取れる。**
  //    2026-08-31 の外部レビュー P0（`docs/guest-mode-spec.md` §23）。
  //
  //    `CLAUDE.md` §5「テストが緑であることを、仕様が正しい根拠にしない」。
  //    **このテストは、誤った側を正解として固定していた実例。**
  test("🔴 通さない（読めない端末は無限の入口になる）", () => {
    const d = decideGuest("attested", { kind: "unknown", reason: "unavailable" });
    assert.equal(
      d.allow,
      false,
      "読めない端末を通している。匿名 ID は作り直せるので DB 側も効かない",
    );
    assert.equal(
      d.writeBack,
      false,
