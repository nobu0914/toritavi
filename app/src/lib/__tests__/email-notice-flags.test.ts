...
// ---------------------------------------------------------------------------
// 🔴 文面の日英は、フラグが閉じていても見る（JR000255・2026-09-16）
//
// 上の「有効にするなら」の検査は `EMAIL_CHANGE_NOTICE_ENABLED = false` の
// あいだ **return して何も見ない**。閉じているうちに文面が片方へ戻されても
// 気づけない —— 開ける人は手順を読んで開けるので、**開けた瞬間に初めて
// 赤くなる**形だと、その場で作業が止まる。だからここは常に見る。
// ---------------------------------------------------------------------------
test("🔴 文面は日英とも実在し、lang で入れ替わる（閉じていても見る）", () => {
  const c = code(ROUTE);
  assert.ok(
    c.includes('meta["lang"] === "en"'),
    "🔴 `raw_user_meta_data.lang` を見ていない。認証メール 4 本と源が揃わない",
  );
  assert.ok(
    c.includes("[JUNROS] A request to change your email address"),
    "🔴 英語の件名が無い",
  );
  assert.ok(
    c.includes("【JUNROS】メールアドレスの変更が要求されました"),
    "🔴 日本語の件名が無い",
  );
  assert.ok(
    c.includes("Coyote and Powell LLC"),
    "🔴 英語の署名が無い（英語の法務ページの表記に揃える）",
  );
});

test("🔴 言語をクライアントから受け取っていない", () => {
  // 宛先と同じ理由（オープンリレー化）。言語を送れると、第三者が
  // 本人の受け取る言語を選べる。
  const c = code(ROUTE);
  assert.ok(
    !c.includes("request.json("),
    "🔴 リクエストの本文を読んでいる。宛先も言語もサーバの値だけを使うこと",
  );
});
