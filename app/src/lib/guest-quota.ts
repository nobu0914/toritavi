/**
 * 🔴 **ゲスト（未登録）での利用は提供しない**（2026-08-31 に利用者が決定）。
 *
 * **アプリ側のフラグ（`kGuestModeEnabled`）ではサーバは閉じない。**
 * サーバは匿名 JWT を受けるので、Supabase の匿名サインインが有効な限り
 * 外部から使えてしまう。**ダッシュボードの設定だけに頼らない** ——
 * 誰かが再び有効化した瞬間に穴が開く。ここで断る。
 *
 * 外部レビュー（2026-08-31）で見つかった、戻す前に必ず塞ぐもの:
 *
 * | | |
 * |---|---|
 * | **P0** | 端末トークンが無い／読めないと `decideGuest` が `allow` を返す。**匿名 ID を作り直せば無限に取れる**（実測で踏んだ）|
 * | **P0** | `setGuestUsed` の失敗を無視して続行する。書けなくても OCR が通る |
 * | **P0** | `decision.remaining` と実際の `units` を比較していない。残り 1 件でも 3 ページを 1 要求で通せる |
 * | **P1** | 匿名は `authenticated` ロール。**既存の RLS はどれも匿名を区別していない** —— Storage も旅程も直接叩ける（実測で作成できた）|
 * | **P1** | App Attest の assertion を要求ごとに検証していない。保存した公開鍵が使われていない |
 * | **P1** | `environment` を検証結果ではなく `ALLOW_DEV` から記録している。development の grant が残る |
 * | **P1** | 匿名の `auth.users` を誰も消さない（029 は旅程に `deleted_at` を立てるだけ）|
 *
