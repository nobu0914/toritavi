 ...
 *
 * ## 2026-09-04 に開け、**同日に閉じ直した**（利用者判断）
 *
 * 🔴 **閉じたのは欠陥が出たからではない。** 実装・法務文書とも揃った状態で、
 * **事業判断として短期保留**にした。決め手は 2 つ ——
 * ①**増分効果を測れない**（登録ファネルの計測が無く、ゲスト経由の Pro が
 * 「ゲストが無くても Pro になった人」かを区別できない）
 * ②**共有 MAU の残量が不明**（匿名も 1 MAU。Supabase の枠は別サービスと
 * 共用で、超えると向こうにも波及する）。
 * 経緯と再開の条件は `toritavi_app/docs/guest-mode-spec.md` §24。
 *
 * 🔴 **アプリの導線を閉じるだけでは足りない。** ここと Supabase の匿名
 * サインインも閉じること。開いたままだと**公開 API から匿名アカウントを
 * 作れる**ので、**MAU を消費されるのに利益はゼロ**という最悪の組み合わせに
 * なる。
 *
 * ## 開けたときの記録（2026-09-04・数時間）
 *
